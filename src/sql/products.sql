...
-- 新table 判斷更新有子類column
UPDATE products_malbic_partial_column 
SET 有子類 = CASE
    WHEN 商品編號 IN (SELECT 商品編號 FROM products_malbic_partial_column GROUP BY 商品編號 HAVING COUNT(*) > 1)
    THEN TRUE
    ELSE FALSE
END;

-- 新table 將產品寫到shopline
...
FROM products_malbic_partial_column;
