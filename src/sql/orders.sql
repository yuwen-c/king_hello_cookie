...
-- 另外存交易序號到一個table
create table orders_transaction_id(
	交易序號 varchar(50)
  交易平台交易序號 varchar(255);
)

-- 把兩個欄位組合，當成判斷訂單的唯一值
-- ALTER TABLE orders_malbic
-- ADD COLUMN 交易平台交易序號 VARCHAR(255);

UPDATE orders_malbic
SET 交易平台交易序號 = 交易平台 || 交易序號;

-- 將所有不同的(distinct)交易序號挑出來，插入到 orders_transaction_id 表格中
insert into orders_id_platform ("交易平台交易序號")
select distinct 交易平台交易序號 from orders_malbic;

-- 處理收件者手機，0被去掉的問題，限定2023年的訂單
UPDATE orders_malbic
SET "收件者手機" = 
   CASE 
