...
-- 初步顧客資料格式轉換，先把地址塞在original_address裡面，還沒拆
...
SELECT
    ...
    'Y', -- 寫為會員
    CONCAT('舊站-會員級別-', customers_mailbic_mandarin.會員等級, ',') AS tags, -- 所有要加的tag前面都要加上「舊站-」
    case when 會員備註 = '' then null else CONCAT('舊站-會員備註-', customers_mailbic_mandarin.會員備註, ',') end,
    null,  -- 交易總金額應該直接寫null
    NULLIF(紅利點數, 0), -- 使用 NULLIF 將為 0 的值轉為 NULL
    NULLIF(購物金, 0),   
    地址
FROM customers_mailbic_mandarin;


-- 把original_address有門市的刪
...
-- 如果address_city有值，再去寫地址欄位。address_1會把縣市刪除
UPDATE customers_shopline
...
-- 把電話寫到備註，接在原本的備註後面。之後再把有問題的電話刪除。
update customers_shopline 
set note case 
...
-- 把重複的電話刪除，只留下email比較小的-----不然如果有重複的電話，會兩筆都無法匯入shopline
UPDATE customers_shopline AS c
SET mobile_number = NULL
WHERE EXISTS (
   SELECT 1
   FROM customers_shopline AS c2
